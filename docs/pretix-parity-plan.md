...
| X5 | **Shape drawing** ✅ shipped | ✅ Rectangle + ellipse (drag to draw, live rubber-band) and text (click to place). ✅ **Pen tool (polygon + bezier curves)** — vector-app experience pretix can't match: click for corners, click-drag to pull symmetric bezier handles, close by clicking the first point / double-click / Enter, Esc cancels; live preview with anchors + handle lines. Curves tessellate into dense `polygon.points` for TipTip/pretix compat while editable `polygon.nodes` (with handles) persist for re-open. `engine/pen.ts` (tessellate/tracePath/bbox) + `makePolygonArea`. All shapes auto-select into the style panel, one undo step. ✅ **Arrange / stacking order** (June 2026): To front / To back / Forward / Backward in the shape panel (`reorderArea` in ops); seats always paint after areas so they stay in front by design. ✅ **Rotation fixed** — rectangles now actually rotate (render, hit-test, resize handles, highlight all rotation-aware); the shape panel's Rotation field persists correctly (rewrote `commitObjectProp` to deep-clone + re-select by index instead of hit-testing, which broke once a shape rotated out from under its anchor). Remaining: re-editing existing curve handles on-canvas. | P1 | L |
