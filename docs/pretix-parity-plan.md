...
| N4 | **Category UX** (mostly shipped) | ✅ Alias + counts + assignment merged into one Categories panel (the separate "Category Update" list is gone — select seats, click Assign on the card). ✅ Duplicate category names are blocked on rename, warned on upload, and recoverable (renaming a duplicate out leaves seats with the remaining one — they were indistinguishable by name). Remaining: color editing, create/delete category, click-to-highlight a category's seats. | P1 | S | 432 | Performance |
