...
| X5 | **Shape drawing** 🟡 in progress | ✅ Rectangle + ellipse (drag to draw, live violet rubber-band, tiny drag = default size) and standalone text (click to place, defaults to "Label") — toolbar Square/Circle/Type tools, Esc cancels, new shape auto-selects into the style panel (fill/border/rotation/text already there), one undo step, resize handles from X2 apply immediately. Factories + `addArea` in `model/ops.ts`. Remaining: polygon (click-to-place vertices, drag to edit, double-click to finish), stacking order. | P1 | L |
