...
| X2 | **Object transform + contextual sidebar** 🟡 in progress | ✅ Contextual sidebar (shipped earlier: plan / row / seat / shape panels). ✅ Move single objects, keyboard nudge, delete key. ✅ **On-canvas resize handles** (`engine/transform.ts`, June 2026): 8 handles on rectangles/ellipses, 4 corner handles on circles (uniform), opposite edge/corner stays fixed, rotation-aware (correct on rotated ellipses), min-size clamp, screen-constant handle size, hover resize-cursors, one undo step per drag, panel dimension fields sync on release. Verified in-browser + 14 unit assertions on the box math. ✅ **Shift multi-select** (June 2026): shift-click seats to toggle, shift-drag marquee to extend, drag any selected seat to move the whole group (one undo step); multi-seat selection drives the existing status/category/bend/delete/nudge panel. Remaining: rotate handle, multi-shape (area) selection. | P0 | M–L |
...
| X5 | **Shape drawing** ✅ shipped | ✅ Rectangle + ellipse (drag to draw, live rubber-band) and text (click to place). ✅ **Pen tool (polygon + bezier curves)** — vector-app experience pretix can't match: click for corners, click-drag to pull symmetric bezier handles, close by clicking the first point / double-click / Enter, Esc cancels; live preview with anchors + handle lines. Curves tessellate into dense `polygon.points` for TipTip/pretix compat while editable `polygon.nodes` (with handles) persist for re-open. `engine/pen.ts` (tessellate/tracePath/bbox) + `makePolygonArea`. All shapes auto-select into the style panel, one undo step. ✅ **Arrange / stacking order**: compact icon row (To back / backward / forward / To front) in the shape panel (`reorderArea`); seats always paint after areas so they stay in front by design. ✅ **Rotation fixed** — rectangles rotate (render, hit-test, resize handles, highlight all rotation-aware); shape-panel fields persist via a rewritten `commitObjectProp` (deep-clone + index re-select). ✅ **On-canvas curve editing** (June 2026): selecting a curved polygon shows its anchors + bezier handles; drag an anchor (snaps to peers/grid, handles follow) or a handle (adjusts the curve) — re-tessellates `points` live so render/export stay in sync. Done. | P1 | L |
| X6 | **Copy / paste / duplicate** ✅ shipped | ⌘C / ⌘V / ⌘D plus Duplicate buttons in the shape + selection panels. Works for a multi-seat selection (grouped into one new row as a movable block), a single seat, a shape/area, or a row. Pasted copies get fresh uuids/guids, are offset +24px, and become the new selection. Rows get a smart next label (A→B, Z→AA via `nextRowLabel`). Verified: shape dup (fresh uuid, +24 offset); 120 seats → 240 with all-unique guids. | P1 | M |
